...
fn client() -> Result<Client, String> {
    Client::builder()
        ...
        .build()
        .map_err(|e| format!("Could not initialize Canvas HTTP client: {e}"))
}
...
fn load(app: &AppHandle) -> Result<Option<CanvasCache>, String> {
    ...
    serde_json::from_slice(&bytes)
        .map(Some)
        .map_err(|e| format!("Canvas credential cache was invalid: {e}"))
}

fn send(cache: &CanvasCache, url: &str) -> Result<Response, String> {
    let response = client()?
        .get(url)
        .bearer_auth(&cache.token)
        .send()
        .map_err(|e| format!("Canvas request failed: {e}"))?;
    if !response.status().is_success() {
        return Err(format!(
            "Canvas request failed (HTTP {}). Check the token, permissions, and Canvas URL.",
            response.status()
        ));
    }
    Ok(response)
...
fn get_all(cache: &CanvasCache, url: String) -> Result<Vec<Value>, String> {
    ...
    for _ in 0..100 {
        ...
        let page: Value = response
            .json()
            .map_err(|e| format!("Canvas returned invalid JSON: {e}"))?;
        if let Some(values) = page.as_array() {
            result.extend(values.iter().cloned());
        } else {
            return Err("Canvas list endpoint did not return an array.".into());
        }
    }
    Ok(result)
}
...
pub fn connect(app: &AppHandle, base_url: &str, token: &str) -> Result<CanvasAccount, String> {
    if token.trim().len() < 8 {
        return Err("Canvas token appears incomplete.".into());
    }
    let base = normalize_base(base_url)?;
    let temp = CanvasCache {
        base_url: base.clone(),
        token: token.trim().to_string(),
        account_label: String::new(),
    };
    let profile: Value = send(&temp, &format!("{base}/api/v1/users/self/profile"))?
        .json()
        .map_err(|e| format!("Canvas profile response was invalid: {e}"))?;
    let label = profile
        .get("name")
        .and_then(Value::as_str)
        .or_else(|| profile.get("short_name").and_then(Value::as_str))
        .unwrap_or("Canvas account")
        .to_string();
    save(
        app,
        &CanvasCache {
...
pub fn status(app: &AppHandle) -> Result<CanvasStatus, String> {
    Ok(match load(app)? {
        Some(cache) => CanvasStatus {
            connected: true,
            account_label: Some(cache.account_label),
            base_url: Some(cache.base_url),
        },
        None => CanvasStatus {
            connected: false,
            account_label: None,
...
fn infer_year(month: u32, day: u32, explicit: Option<i32>) -> Option<chrono::NaiveDate> {
    ...
    if date < today - chrono::Duration::days(120) {
...
fn date_candidates(text: &str) -> Vec<DateCandidate> {
    ...
    for c in named.captures_iter(text) {
        let Some(full) = c.get(0) else {
            continue;
        };
        let Some(month_match) = c.get(1) else {
            continue;
        };
        let Some(day_match) = c.get(2) else {
            continue;
        };
        let Some(month) = month_number(month_match.as_str()) else {
            continue;
        };
        let Ok(day) = day_match.as_str().parse::<u32>() else {
            continue;
        };
        let year = c.get(3).and_then(|m| m.as_str().parse::<i32>().ok());
        let Some(date) = infer_year(month, day, year) else {
            continue;
        };
    ...
    for c in numeric.captures_iter(text) {
        ...
        let Ok(month) = month_match.as_str().parse::<u32>() else {
            continue;
        };
        let Ok(day) = day_match.as_str().parse::<u32>() else {
            continue;
...
fn detect_date_near_assignment(syllabus: &str, assignment_name: &str) -> Option<String> {
    ...
    if assignment_name.trim().len() < 4 || syllabus.trim().is_empty() {
        return None;
    }
    ...
    for (assignment_start, _) in lower.match_indices(&needle) {
        ...
        while start < plain.len() && !plain.is_char_boundary(start) {
            start += 1;
        }
        while end > start && !plain.is_char_boundary(end) {
            end -= 1;
        }
        let Some(window) = plain.get(start..end) else {
            continue;
        };
...
fn module_assignment_hints(cache: &CanvasCache, course_id: &str) -> HashMap<String, String> {
    ...
    for module in get_all_best_effort(cache, modules_url) {
        ...
        for item in items {
            if item.get("type").and_then(Value::as_str) != Some("Assignment") {
                continue;
            }
            let content_id = item
                .get("content_id")
            ...
            if !content_id.is_empty() && !title.is_empty() {
                hints.insert(content_id, title.to_string());
            }
        }
    }
    hints
...
) -> Result<Vec<CanvasAssignmentRecord>, String> {
    let cache = load(app)?.ok_or_else(|| {
        "Canvas is not connected. Paste the token once in Settings first.".to_string()
    })?;
    let mut result = Vec::new();
    for student in students {
        if student.student_id.trim().is_empty() || student.canvas_user_id.trim().is_empty() {
            continue;
        }
        ...
        for course in courses {
            ...
            let assignments_url = format!(
                "{}/api/v1/users/{}/courses/{}/assignments?per_page=100",
                cache.base_url,
                urlencoding::encode(&student.canvas_user_id),
                urlencoding::encode(&course_id)
            );
            ...
            for assignment in assignments {
                ...
                let module_hint = module_hints
                    .get(&assignment_id)
                    .map(String::as_str)
                    .unwrap_or_default();
                let (possible, source) = if official.is_some() {
                    (None, None)
                } else if let Some(date) = detect_date(&name) {
                    (Some(date), Some("title".to_string()))
                } else if let Some(date) = detect_date(&description) {
                    (Some(date), Some("description".to_string()))
                } else if let Some(date) = detect_date(module_hint) {
                    (Some(date), Some("module".to_string()))
                } else if let Some(date) = detect_date_near_assignment(&syllabus_body, &name) {
                    (Some(date), Some("syllabus".to_string()))
                } else {
                    (None, None)
                };
                result.push(CanvasAssignmentRecord {
                    student_id: student.student_id.clone(),
                    student_canvas_id: student.canvas_user_id.clone(),
                    course_name: course_name.clone(),
                    course_id: course_id.clone(),
                    assignment_name: name,
                    assignment_id,
                    official_due_date: official,
                    possible_due_date: possible,
                    detected_date_source: source,
                    canvas_url: assignment
                        .get("html_url")
                        .and_then(Value::as_str)
                        .map(str::to_string),
...
mod tests {
    ...
    fn explicit_named_date_is_detected() {
        assert_eq!(
            detect_date("Puppet Video due September 16, 2026").as_deref(),
            Some("2026-09-16")
        );
    }
    ...
    fn syllabus_detection_requires_assignment_neighborhood() {
        ...
        assert_eq!(
            detect_date_near_assignment(syllabus, "Chemistry Test").as_deref(),
            Some("2026-09-18")
        );
        assert_eq!(
            detect_date_near_assignment(syllabus, "Unlisted Assignment"),
            None
        );
    }
    ...
    fn syllabus_detection_accepts_date_before_assignment() {
        let syllabus =
            "Week 3: September 18, 2026 — Chemistry Test. Week 4: September 25, 2026 — Lab.";
        assert_eq!(
            detect_date_near_assignment(syllabus, "Chemistry Test").as_deref(),
            Some("2026-09-18")
        );
