...
pub fn identity_public(app: &AppHandle) -> Result<IdentityPublic, String> {
    Ok(IdentityPublic {
        public_key: identity(app)?.public_key,
    })
}
...
fn shared_key(
    ...
    let private_bytes: [u8; 32] = STANDARD
        .decode(local.private_key)
        .map_err(|_| "Local sync private key is unreadable.".to_string())?
        .try_into()
        .map_err(|_| "Local sync private key has the wrong length.".to_string())?;
    let peer_bytes: [u8; 32] = STANDARD
        .decode(peer_public_key)
        .map_err(|_| "Peer synchronization public key is unreadable.".to_string())?
        .try_into()
        .map_err(|_| "Peer synchronization public key has the wrong length.".to_string())?;
...
pub fn seal_envelope(
    ...
    let ciphertext = cipher
        .encrypt(
            ...
            Payload {
                msg: &plaintext,
                aad: mailbox_id.as_bytes(),
            },
        )
        .map_err(|_| "Could not encrypt synchronization envelope.".to_string())?;
    ...
    serde_json::to_string(&packet)
        .map_err(|e| format!("Could not encode encrypted synchronization packet: {e}"))
}
...
pub fn open_packet(
    ...
    if decoded.sender_public_key != peer_public_key {
        return Err("Synchronization packet sender did not match the paired device.".into());
    }
    let key = shared_key(app, peer_public_key, mailbox_id)?;
    let nonce: [u8; 12] = STANDARD
        .decode(decoded.nonce)
        .map_err(|_| "Synchronization nonce was unreadable.".to_string())?
        .try_into()
        .map_err(|_| "Synchronization nonce was the wrong length.".to_string())?;
    let ciphertext = STANDARD
        .decode(decoded.ciphertext)
        .map_err(|_| "Synchronization ciphertext was unreadable.".to_string())?;
    let cipher = ChaCha20Poly1305::new_from_slice(&key)
        .map_err(|_| "Could not initialize sync decryption.".to_string())?;
    let plaintext = cipher
        ...
        .map_err(|_| "Synchronization packet failed authentication/decryption.".to_string())?;
    serde_json::from_slice(&plaintext)
        .map_err(|_| "Decrypted synchronization packet was not valid JSON.".to_string())
}
...
fn ensure_share_dirs(root: &Path, mailbox_id: Option<&str>) -> Result<(), String> {
    for part in [
        ...
        fs::create_dir_all(root.join(part))
            .map_err(|e| format!("Could not access university sync directory {part}: {e}"))?;
    }
    if let Some(mailbox) = mailbox_id {
        validate_id(mailbox)?;
        for part in ["student-to-coordinator", "coordinator-to-student"] {
            fs::create_dir_all(root.join("mailboxes").join(mailbox).join(part))
                .map_err(|e| format!("Could not access mailbox transport: {e}"))?;
        }
    }
    Ok(())
}
...
pub fn create_pair_invite(
    ...
    let invite = PairInvite {
        ...
        mailbox_id: mailbox_id.to_string(),
        coordinator_public_key: identity(app)?.public_key,
        expires_at: expires.to_rfc3339(),
        created_at: now_iso(),
    };
    storage::atomic_write(
        &root.join("pairing/invites").join(format!("{hash}.json")),
        &serde_json::to_vec_pretty(&invite).map_err(|e| e.to_string())?,
    )?;
    Ok(PairCodeResult {
        code,
...
pub fn request_pairing(
    ...
    code: &str,
    device_id: &str,
) -> Result<PairRequestResult, String> {
    ...
    let request = LocalPairRequest {
        ...
        student_public_key: identity(app)?.public_key,
        created_at: now_iso(),
    };
    storage::atomic_write(
        &root
            .join("pairing-requests")
            .join(format!("{request_id}.json")),
        &serde_json::to_vec_pretty(&request).map_err(|e| e.to_string())?,
    )?;
    Ok(PairRequestResult {
        request_id,
...
pub fn pairing_response() -> Result<Option<PairResponse>, String> {
    ...
    let mut files: Vec<_> = fs::read_dir(&dir)
        .map_err(|e| format!("Could not read local pairing responses: {e}"))?
        .flatten()
        .collect();
    ...
    let response = serde_json::from_slice(&fs::read(entry.path()).map_err(|e| e.to_string())?)
        .map_err(|e| format!("Pairing response was invalid: {e}"))?;
    Ok(Some(response))
}
...
pub fn read_pair_requests(share_root: &str) -> Result<Vec<PairRequest>, String> {
    ...
    for entry in fs::read_dir(root.join("pairing/requests"))
        .map_err(|e| format!("Could not read pairing requests: {e}"))?
        .flatten()
    {
        ...
        if let Ok(request) =
            serde_json::from_slice::<PairRequest>(&fs::read(entry.path()).unwrap_or_default())
        {
            result.push(request);
        }
    }
...
fn valid_direction(direction: &str) -> Result<&str, String> {
    match direction {
        "student-to-coordinator" | "coordinator-to-student" => Ok(direction),
        _ => Err("Invalid synchronization direction.".into()),
    }
}
...
pub fn drive_send(
    ...
    let direction = valid_direction(direction)?;
    let root = PathBuf::from(share_root);
    ensure_share_dirs(&root, Some(mailbox_id))?;
    storage::atomic_write(
        &root
...
pub fn drive_receive(
    share_root: &str,
    mailbox_id: &str,
    direction: &str,
) -> Result<Vec<PacketFile>, String> {
    validate_id(mailbox_id)?;
    let direction = valid_direction(direction)?;
    let root = PathBuf::from(share_root);
    ensure_share_dirs(&root, Some(mailbox_id))?;
        ...
        .flatten()
    {
        ...
        if let Ok(packet) = fs::read_to_string(entry.path()) {
            result.push(PacketFile {
                file_name: name,
                packet,
            });
        }
...
pub fn test_share(share_root: &str) -> Result<String, String> {
    if share_root.trim().is_empty() {
        return Err("University synchronization drive path is empty.".into());
    }
    let root = PathBuf::from(share_root);
    ensure_share_dirs(&root, None)?;
...
pub fn drive_ack(
    ...
    validate_id(mailbox_id)?;
    let direction = valid_direction(direction)?;
    if Path::new(file_name).file_name().and_then(|x| x.to_str()) != Some(file_name) {
        return Err("Invalid sync file name.".into());
    }
    let path = PathBuf::from(share_root)
        .join("mailboxes")
        .join(mailbox_id)
        .join(direction)
        .join(file_name);
    if path.exists() {
        fs::remove_file(path).map_err(|e| format!("Could not acknowledge sync packet: {e}"))?;
    }
    Ok(())
}
...
pub fn spool_ack(file_name: &str) -> Result<(), String> {
    if Path::new(file_name).file_name().and_then(|x| x.to_str()) != Some(file_name) {
        return Err("Invalid local sync file name.".into());
    }
    let root = ensure_spool()?;
...
fn read_agent_config() -> Result<Option<AgentConfig>, String> {
    ...
    let encrypted =
        fs::read(path).map_err(|e| format!("Could not read managed sync configuration: {e}"))?;
    let bytes = storage::protect::unprotect_machine(&encrypted)?;
    serde_json::from_slice(&bytes)
        .map(Some)
        .map_err(|e| format!("Managed sync configuration was invalid: {e}"))
}
fn write_agent_config(config: &AgentConfig) -> Result<(), String> {
    fs::create_dir_all(agent_root())
        .map_err(|e| format!("Could not create managed sync configuration directory: {e}"))?;
    let bytes = serde_json::to_vec(config).map_err(|e| e.to_string())?;
    storage::atomic_write(
        &agent_config_path(),
        &storage::protect::protect_machine(&bytes)?,
    )
}
...
pub fn configure_agent_cli(encoded_root: &str) -> Result<(), String> {
    let bytes = URL_SAFE_NO_PAD
        .decode(encoded_root)
        .map_err(|_| "Managed sync path argument was invalid.".to_string())?;
    let root =
        String::from_utf8(bytes).map_err(|_| "Managed sync path was not UTF-8.".to_string())?;
    if root.trim().is_empty() {
        return Err("Managed sync path cannot be empty.".into());
    }
    fs::create_dir_all(agent_root())
        .map_err(|e| format!("Could not create managed sync directory: {e}"))?;
    let existing = read_agent_config()?.unwrap_or_default();
    write_agent_config(&AgentConfig {
        version: 1,
        share_root: root,
        ..existing
    })
...
pub fn configure_agent_elevated(share_root: &str) -> Result<bool, String> {
    ...
    let args_w = wide(OsStr::new(&args));
    let result = unsafe {
        ShellExecuteW(
            std::ptr::null_mut(),
            verb.as_ptr(),
    ...
    if result <= 32 {
        return Err(format!(
            "Windows could not elevate managed sync configuration (ShellExecute code {result})."
        ));
    }
    Ok(true)
}
#[cfg(not(target_os = "windows"))]
pub fn configure_agent_elevated(_share_root: &str) -> Result<bool, String> {
...
fn write_agent_status(config: Option<&AgentConfig>, message: &str) {
    if let Ok(root) = ensure_spool() {
        let status = AgentStatusFile {
            ...
            paired_mailbox_id: config.and_then(|c| c.paired_mailbox_id.clone()),
...
fn process_pairing(config: &mut AgentConfig) -> Result<usize, String> {
    ...
    ensure_share_dirs(&root, None)?;
    let mut count = 0;
    for entry in fs::read_dir(spool.join("pairing-requests"))
        .map_err(|e| e.to_string())?
        .flatten()
    {
        let bytes = fs::read(entry.path()).unwrap_or_default();
        let Ok(local) = serde_json::from_slice::<LocalPairRequest>(&bytes) else {
            continue;
        };
        let invite_path = root
            .join("pairing/invites")
            .join(format!("{}.json", local.code_hash));
        if !invite_path.exists() {
            continue;
        }
        let invite: PairInvite =
            serde_json::from_slice(&fs::read(&invite_path).map_err(|e| e.to_string())?)
                .map_err(|e| format!("Pairing invite was invalid: {e}"))?;
        let expiry = chrono::DateTime::parse_from_rfc3339(&invite.expires_at)
            .map_err(|_| "Pairing invite expiration was invalid.".to_string())?;
        if expiry.with_timezone(&chrono::Utc) < chrono::Utc::now() {
            continue;
        }
        let mailbox_changed = config
            .paired_mailbox_id
            .as_deref()
            .is_some_and(|existing| existing != invite.mailbox_id.as_str());
        let used = root
            .join("pairing/used")
            .join(format!("{}.{}.json", invite.code_hash, local.device_id));
        if let Some(parent) = used.parent() {
            let _ = fs::create_dir_all(parent);
        }
        if fs::rename(&invite_path, &used).is_err() {
            continue;
        }
        let request = PairRequest {
            request_id: local.request_id.clone(),
            code_hash: local.code_hash.clone(),
            student_id: invite.student_id.clone(),
            mailbox_id: invite.mailbox_id.clone(),
            device_id: local.device_id.clone(),
            student_public_key: local.student_public_key.clone(),
            created_at: local.created_at.clone(),
        };
        storage::atomic_write(
            &root
                .join("pairing/requests")
                .join(format!("{}.json", local.request_id)),
            &serde_json::to_vec_pretty(&request).map_err(|e| e.to_string())?,
        )?;
        let response = PairResponse {
            ...
            student_id: invite.student_id.clone(),
            mailbox_id: invite.mailbox_id.clone(),
            coordinator_public_key: invite.coordinator_public_key,
            expires_at: invite.expires_at,
        };
        storage::atomic_write(
            &spool
                .join("pairing-responses")
                .join(format!("{}.json", local.request_id)),
            &serde_json::to_vec_pretty(&response).map_err(|e| e.to_string())?,
        )?;
        if mailbox_changed {
        ...
        let _ = fs::rename(
            entry.path(),
            spool.join("processed").join(entry.file_name()),
        );
        count += 1;
    }
    Ok(count)
...
fn process_agent_transport(config: &AgentConfig) -> Result<(usize, usize, usize), String> {
    let Some(mailbox) = config.paired_mailbox_id.as_deref() else {
        return Ok((0, 0, 0));
    };
    validate_id(mailbox)?;
    let spool = ensure_spool()?;
    ...
    for entry in fs::read_dir(spool.join("outgoing"))
        .map_err(|e| e.to_string())?
        .flatten()
    {
        let Ok(file) =
            serde_json::from_slice::<SpoolPacket>(&fs::read(entry.path()).unwrap_or_default())
        else {
            continue;
        };
        if file.mailbox_id != mailbox {
            continue;
        }
        drive_send(
            &config.share_root,
    ...
    for entry in fs::read_dir(remote).map_err(|e| e.to_string())?.flatten() {
        ...
        if !local.exists() {
            fs::copy(entry.path(), &local)
                .map_err(|e| format!("Could not copy incoming sync packet locally: {e}"))?;
            received += 1;
        }
    }

    for entry in fs::read_dir(spool.join("acks"))
        .map_err(|e| e.to_string())?
        .flatten()
    {
        let Some(stem) = entry
...
fn embedded_default_share_root() -> Option<String> {
    let raw: serde_json::Value =
        serde_json::from_str(include_str!("../deployment.defaults.json")).ok()?;
...
pub fn run_agent() -> Result<(), String> {
    ...
    match read_agent_config() {
        ...
        Ok(_) => {
            write_agent_status(
                None,
                "Managed sync agent is installed and waiting for university drive configuration.",
            );
            Ok(())
        }
        Err(error) => {
